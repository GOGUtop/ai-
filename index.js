...
async function request(path,body,absolute=false){
  const response=await fetch(absolute?path:'/api/plugins/writer-ai-relay-server'+path,{method:body?'POST':'GET',credentials:'same-origin',headers:{...(ctx()?.getRequestHeaders?.()||{}),'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined});
  const raw=await response.text();let result;
  try{result=JSON.parse(raw);}catch{throw new Error(response.status===404?'AI 接力服务端未加载（HTTP 404），请安装 writer-ai-relay-server 并重启酒馆':`酒馆服务端返回非 JSON（HTTP ${response.status}），请检查登录、反向代理和服务端安装`);}
  if(!response.ok||result?.ok===false)throw new Error(result.error||`请求失败 HTTP ${response.status}`);
  return result;
}
